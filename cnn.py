...
batch_size=128
a = np.loadtxt('data/sleep/pressure/experiment1/data.csv')
a=a[:,0:32*16]
b = np.loadtxt('data/sleep/pressure/experiment1/label.csv',dtype=np.int32)
b=b-1
...
print(pool2)
#flat(平坦化)
flat=tf.reshape(pool2,[-1,8*4*64])
...
test_accuracy=sess.run(accuracy_op,{input_x:test_x,output_y:label_test})
print(test_accuracy)
np.savetxt('e1_y_score.csv',test_output)
np.savetxt('e1_test_score.csv',label_test)
